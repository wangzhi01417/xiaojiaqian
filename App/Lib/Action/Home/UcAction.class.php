...
class UcAction extends BaseAction {
	...
	function sinacallback() {
		...
		if ($user_openid) {
			...
			$data = array (
					...
					'profile_image_url' => $ret['avatar_large'],
	...
	function renrenLogin(){
		...
		$domain = 'http://'.$_SERVER['HTTP_HOST'];
		$redirect_uri = $domain.get_url('renren'.$type, '', 'user');
		$login_url = "https://graph.renren.com/oauth/authorize?client_id=".c('renren_apikey')."&redirect_uri=".urlencode($redirect_uri)."&response_type=code"."&state=".$_SESSION['state'];
		header("Location:$login_url");
	}

	function renrencallback(){
		echo "renrencallback";
		$type = isset ( $_REQUEST ['type'] ) ? $_REQUEST ['type'] : 'callback';
		if ($_REQUEST['state'] == $_SESSION['state']) {
			if ($_REQUEST['code']) {
				...
				$params = array(
					...
					"code" => $_REQUEST['code'],
					"client_id" => c('renren_apikey'),
					"client_secret" => c('renren_secretkey'),
					"redirect_uri" => 'http://'.$_SERVER['HTTP_HOST'].get_url('renren'.$type, '', 'user')
				);			
				$res = post( $token_url, $params);
				$res = json_decode($res);
				$user = $res->user;
				
				
				$user_url = "https://api.renren.com/v2/user/get?access_token=".$res->access_token;
				$res = get ( $user_url );

				// $res = post ( $user_url, array(
