...
class ItemAction extends BaseAction {
    public function index(){
		...
		$items_mod= D('Items');		
		$field_items="id,title,cid,sid,price,img,comments,url,likes,hits,add_time,seo_title,seo_keys,seo_desc,status,info,uid,remark1,remark2";
		...
		$items_cate_item=$items_mod->where("id!=$id and cid=$cid and is_del=0 and status=1 and add_time>$daytime")->field($field_items)->order("hits desc")->limit(6)->select();
