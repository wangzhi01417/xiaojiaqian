...
#img_list input{
  ...
  left:-30px;
		...
		$("#collect_item_today").click(function(){
		    ...
		    $.post("<?php echo u('Items/collect_jiukuaiyou_today');?>", { url: url }, function(data){
		...
		$("#submit").click(function(){
			...
			if ($("#info").val().length > 140) {
...
</script></head><body><form action="<?php echo U('Items/collect_between_specific_times');?>" method="post" name="myform" id="myform"  enctype="multipart/form-data" style="margin-top:10px;"><div class="pad-10"><div class="col-tab"><div id="div_setting_1" class="contentList pad-10"><table width="95%" cellspacing="0" class="search-form" align="center"><tbody><tr><td><div class="explain-col">					&nbsp;&nbsp;&nbsp;&nbsp;<font color=red><b>          注意：<br/>          1）先选择一种九块邮的商品类别<br/>          2）然后点击“提交”，采集该种类别今天30种商品。<br/><br/>          *不建议每次选中好几个类别<br/>          *系统目前还处于测试阶段
        </b></font></div></td></tr></tbody></table><div name="error_msg" id="error_msg" style="margin:0px 300px;color:red;display:none;"><b>该商品不存在</b></div><table width="100%" cellpadding="2" cellspacing="1" class="table_form"><tbody id="item_body"><tr><th width="100">采集时间范围: </th><td><input type="text" name="time_start" id="time_start" class="date" size="12" value="<?php echo ($time_start); ?>"><script language="javascript" type="text/javascript">                    Calendar.setup({
                ...
                </script></td></tr><tr><th width="100">采集来源：</th><td><input type="checkbox" name="jiukuaiyou" id="jiukuaiyou" value="true" > 九块邮 &nbsp;&nbsp;
                  <input type="checkbox" name="jiukuaiwu" id="jiukuaiwu" value="true" > 九块屋 &nbsp;&nbsp;
                   <input type="checkbox" name="shikuai" id="shikuai" value="true" > 十块 &nbsp;&nbsp;
              </td></tr><tr><th width="100">采集页数：</th><td><select name="collect_page_cnt" id="collect_page_cnt"><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option></select>                &nbsp;&nbsp;<b><font color="blue">(仅限九块邮)</font></b></td></tr><tr><th width="100">采集类别 :</th><td><!--<input type="text" name="title" id="title" class="input-text not_null" maxlength="140" size="60" /><b style="padding-left:10px;color:red;">*</b>--><input type="checkbox" name="fushi" id="fushi" value="true" > 服饰 &nbsp;&nbsp;
