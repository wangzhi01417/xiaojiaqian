(function() {
	ImportJavascript = {
		url: function(c) {
			...
			b = a.length; d < b; d++) {
				var f = a[d].src;
				if (f && f.indexOf("duitang") > -1) {
					e = f.replace(/^https?:\/\//ig, "").split("/")[0];
					break
				}
			}
			document.write('<script type="text/javascript" src="http://' + e + c + '"><\/script>')
		}
	}
})();
/*
...
(function(a, b) {
	...
	a.Bom = function() {
			...
			if (a.type(l) === "string" && l.length > 0) {
				...
				f = n.length; h < f; h++) {
					...
					if (g instanceof Array) {
						m = unescape(g[1]);
						k = unescape(n[h].substring(g[1].length + 1))
					} else {
						m = unescape(n[h]);
						k = ""
					}
		...
		d = function(h, i, g) {
			...
			var j = escape(h) + "=" + escape(i);
			if (a.isPlainObject(g)) {
				if (g.expires instanceof Date) {
					j += "; expires=" + g.expires.toUTCString()
				}
				if (a.type(g.expires) === "number" && g.expires !== 0) {
					var f = new Date();
					f.setTime(f.getTime() + (g.expires * 24 * 60 * 60 * 1000));
					j += "; expires=" + f.toUTCString()
				}
				if (a.type(g.path) === "string" && g.path !== "") {
					j += "; path=" + g.path
				} else {
					j += "; path=/"
				}
				if (a.type(g.domain) === "string" && g.domain !== "") {
					j += "; domain=" + g.domain
				}
				if (g.secure === true) {
					j += "; secure"
		...
		e = function(g) {
			if (a.isPlainObject(g)) {
				var h = [];
				for (var f in g) {
					if (!a.isFunction(g[f]) && a.type(g[f]) !== "undefined") {
						h.push(escape(f) + "=" + escape(String(g[f])))
					}
				}
				return h.join("|")
			}
		...
		return {
			getCookie: function(f) {
				if (a.type(f) !== "string" || f === "") {
					return null
				}
				var g = c(document.cookie);
				if (a.type(g[f]) === "undefined") {
					return ""
				} else {
					return g[f]
				}
			},
			getSubCookie: function(g, f) {
				var h = this.getSubCookies(g);
				if (h != null) {
					if (a.type(f) !== "string" || f === "") {
						return ""
					} else {
			...
			getSubCookies: function(f) {
				...
				var g = this.getCookie(f);
				if (a.type(g) === "string") {
					return b(g)
				}
				return ""
			},
			setCookie: function(g, h, f) {
				f = f || {};
				if (a.type(g) === "string" && a.type(h) !== "undefined") {
					var i = d(g, h, f);
					document.cookie = i;
					return i
				}
			},
			setSubCookie: function(g, i, h, f) {
				if (a.type(g) !== "string" || g === "") {
					return
				}
				if (a.type(i) !== "string" || i === "") {
					return
				}
				if (a.type(h) === "undefined") {
					return
				}
				var j = this.getSubCookies(g);
				if (!a.isPlainObject(j)) {
					j = {}
				}
				j[i] = h;
				return this.setSubCookies(g, j, f)
			},
			setSubCookies: function(g, h, f) {
				if (a.type(g) === "string" && a.type(h) !== "undefined") {
					var i = d(g, e(h), f);
					document.cookie = i;
					return i
			...
			removeCookie: function(g, f) {
				if (a.type(g) === "string" && !g === false) {
					f = a.extend(f || {},
					{
						expires: new Date(0)
					});
					return this.setCookie(g, "", f)
				}
			},
			removeSubCookie: function(f, h) {
				if (a.type(f) !== "string" || f === "") {
					return
				}
				if (a.type(h) !== "string" || h === "") {
					return
				}
				var g = this.getSubCookies(f);
				if (a.isPlainObject(g) && g.hasOwnProperty(h)) {
					delete g[h];
					return this.setSubCookies(f, g)
				}
			},
			addBookmark: function(f, g) {
				if (window.sidebar) {
					window.sidebar.addPanel(g ? g: document.title, f ? f: window.location.href, "")
				} else {
					if (window.external) {
						window.external.AddFavorite(f ? f: location.href, g ? g: document.title)
					} else {
					...
					if (window.sidebar) {
						if (window.netscape) {
							try {
								...
								alert("璇ユ搷浣滆�娴忚鍣ㄦ嫆缁濓紝鍋囧鎯冲惎鐢ㄨ鍔熻兘锛岃鍦ㄥ湴鍧€鏍忓唴杈撳叆 about:config,鐒跺悗灏嗛」 signed.applets.codebase_principal_support 鍊艰涓簍rue")
							}
						}
...
/*
// Tab Switch jQuery plugin
// copyright balibell, licensed duitang.com
// version 1.0.110523
*/
(function(a) {
	a.fn.tabswitch = function(n, f, k) {
		var i = this;
		...
		function e(w, v) {
			var x = b.sub[0],
			s = b.sub[1];
			for (var u = 0; u < x.length && u < s.length; u++) {
				var t = c.eq(w),
				y = c.eq(v);
				sb = x[u].replace(/\./, ""),
				mn = s[u].replace(/\./, "");
				if (t.hasClass(sb) && y.hasClass(mn)) {
					return 1
				} else {
					if (t.hasClass(mn) && y.hasClass(sb)) {
						return - 1
					}
				}
			}
		...
		function o(v) {
			...
			w = b.sub[0],
			s = b.sub[1];
			for (var t = 0; t < w.length && t < s.length; t++) {
				var y = w[t].replace(/\./, ""),
				x = s[t].replace(/\./, "");
				if (u.hasClass(y) || u.hasClass(x)) {
					return c.filter(w[t])
				}
			}
			return null
		...
		function d(w, t) {
			...
			for (var u = 0; u < s.length; u++) {
				var z = s[u].replace(/\./, "");
				if (v.hasClass(z)) {
					var x = a(y[u]).filter(".cur")[0];
					return t && x ? c.index(x) : c.index(a(y[u])[0])
				}
			}
			return - 1
		}
		function p(w, t) {
			var y = c,
			z, x = c.index(z),
			s = b.sub && b.sub[0] ? b.sub[0].length: null,
			u = mi = -1,
			v = w;
			if (s) {
				y = y.not(b.sub[0].join(","));
				u = d(w, true);
				mi = h(w)
			}
			z = y.filter("." + m);
			x = c.index(z);
			y.add(q).removeClass(m);
			if (s && e(w, x) === 1) {
				o(w).removeClass(m);
				z.addClass(m)
			} else {
			...
			q.css("display", "none").eq(v).css("display", "block").add(c.eq(w)).addClass(m);
			if (b.auto) {
				g()
			}
		...
		function g() {
			...
			l = window.setTimeout(function() {
				var s = c.index(c.filter("." + m));
				p((s + 1) % q.length, true)
			},
			b.autodelay)
		}
		a.fn.tabswitch.stop = function() {
			window.clearTimeout(l)
		};
		a.fn.tabswitch.resume = function() {
			if (b.auto) {
				g()
			}
		};
	...
	a.fn.tabswitch.defaults = {
		cont: ".tabswitch-cont",
		sub: [[".tabswitch-sub0"], [".tabswitch-main0"]],
		focus: "cur",
		index: 0,
		event: "click",
		auto: false,
	...
	a.fn.confirm = function(i, d, c) {
		...
		var b = function(l, k) {
			...
			a(".PL-cfm-cont", j).stop().clearQueue().css({
				marginTop: g * l
			}).animate({
				marginTop: g * k
			},
			300, "linear",
			function() {
				j.css("overflow", "visible").css("display", k > l ? "none": "block")
			})
		};
		var f = function(E) {
			E.preventDefault();
			...
			y = A - v;
			a("html").animate({
				scrollTop: z < k ? z: (z = u + v + B - p) > k ? z: "+=0",
				scrollLeft: y < m ? y: (y = A + v + r - F) > m ? y: "+=0"
			},
			300);
			j.css({
				top: u,
			...
			a(".PL-cfm-wds", j).html(h);
			b(B, 0)
		};
		var j = a("#PL-confirm");
		if (!j.length) {
			j = a('<div id="PL-confirm" class="PL-confirm"><div class="PL-cfm-cont"><div class="PL-cfm-wds tc">' + h + '</div><div class="PL-cfm-btns tc"><a class="abtn abtn-s PL-cfm-yes dib" href="javascript:;""><u>纭畾</u></a><a class="abtn abtn-s PL-cfm-no dib" target="_self" href="javascript:;"><u>鍙栨秷</u></a></div></div></div>').appendTo("body");
			var e = function() {
				var k = j.outerHeight();
				b(0, k)
			};
			a(".PL-cfm-yes", j).click(function(k) {
				k.preventDefault();
				j.data("fn").call(j.data("this"), j.data("event"))
			}).click(e);
			a(".PL-cfm-no", j).click(e)
		}
		if (d) {
			this.delegate(d, "click", f)
		} else {
			this.click(f)
		}
		return this
